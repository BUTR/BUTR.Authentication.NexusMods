namespace BUTR.Authentication.NexusMods.Authentication
{
    public static class ButrNexusModsClaimTypes
    {
        public const string UserId = "NexusMods.UserId";
        public const string Name = "NexusMods.Name";
        public const string EMail = "NexusMods.EMail";
        public const string ProfileUrl = "NexusMods.ProfileUrl";
        public const string IsSupporter = "NexusMods.IsSupporter";
        public const string IsPremium = "NexusMods.IsPremium";
        public const string APIKey = "NexusMods.APIKey";
        public const string Role = "NexusMods.Role";
        public const string Metadata = "NexusMods.Metadata";

        public const string TokenUid = "NexusMods.TokenUid";
        public const string CreationTime = "NexusMods.CreationTime";
    }
}